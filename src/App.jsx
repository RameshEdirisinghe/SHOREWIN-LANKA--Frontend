  ...
  const addToCart = useCallback(
    (product, gradeName, qty, unit, unitPrice) => {
      ...
      setCart((prev) => {
        ...
        if (existing) {
          showToast(
            `Updated ${product.name} (${gradeName}) quantity to ${existing.quantity + qty} ${unit}!`,
          );
        ...
        showToast(
          `Added ${qty} ${unit} of ${product.name} (${gradeName}) to cart.`,
        );
        return [
          ...prev,
          {
  ...
  return (
    <>
      ...
      <main>
        ...
        <Suspense fallback={<PageLoader />}>
          {currentPage === "product-details" && (
            <ProductDetails language={language} products={products} />
          )}
          {currentPage === "about-company" && (
